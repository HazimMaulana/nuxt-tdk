  ...
  <div class="flex flex-col min-h-screen bg-white py-4">
    ...
    <div v-if="tampilanAktif === 'daftar'" class="px-6 mt-12">
      <div class="flex flex-row justify-between items-center w-full">
        <button @click="tampilanAktif = 'buat'"
        ...
        <div class="flex flex-row items-center space-x-3">
          ...
          <div class="dropdown dropdown-hover dropdown-end w-full">
            <div tabindex="0" role="button"
              class="btn m-1 bg-white text-black rounded-4xl shadow-none flex flex-row space-x-2 w-fit">
              <p>Sort By</p>
            ...
            <ul tabindex="0"
              class="dropdown-content menu bg-white text-black border border-black rounded-box z-1 w-52 p-2">
              <li><a>Item 1</a></li>
              <li><a>Item 2</a></li>
            </ul>
          </div>
      ...
      <div class="bg-gray-50 p-8 rounded-lg border">
        <form @submit.prevent="submitPortfolio" class="space-y-4">
          <div>
            ...
            <input v-model="portfolio.title" type="text" id="title"
              class="mt-1 block w-full shadow-sm p-2 text-black border border-gray-700 rounded-2xl" required />
          </div>
          <div>
            ...
            <label for="cover-image-upload"
              ...
              <div class="flex items-center justify-center bg-[#EB5523] text-white p-2 rounded-box">
                ...
                <svg v-else class="w-10 h-10" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none"
                  viewBox="0 0 20 20">
          ...
          <div>
            ...
            <textarea v-model="portfolio.shortDescription" id="shortDescription" rows="4"
              class="mt-1 block w-full shadow-sm p-2 text-black border border-gray-700 rounded-2xl" required></textarea>
          </div>
          <div>
            <label for="description" class="block text-2xl font-bold text-[#EB5523]">Description</label>
            <Editor v-model="portfolio.description" editorStyle="height: 320px" :pt="{
          ...
          <div class="flex justify-end pt-4 space-x-3">
            <!-- <button type="button" @click="tampilanAktif = 'daftar'"
...
<script>
// Impor komponen yang dibutuhkan di sini
import PortoCardAdmin from "../../components/portoCard-admin.vue";
import Editor from 'primevue/editor';
import FormData from 'form-data';
// ... impor komponen lain jika ada

export default {
  components: {
    PortoCardAdmin,
    Editor,
    // Daftarkan komponen lain di sini
  },
  ...
  methods: {
    ...
    async submitPortfolio() {
      ...
      try {
        // GANTI 'api.post' MENJADI 'this.$api.post'
        const response = await this.$api.post("/portfolios/", formData, {
          headers: {
            "Content-Type": "multipart/form-data", // Pastikan ini sesuai dengan format yang diharapkan oleh backend
          },
          // withCredentials tidak perlu lagi di sini jika sudah ada di plugin
        });
        ...
        // if (error.response) {
        //   alert(`Gagal membuat portfolio: ${error.response.data.message || error.response.statusText}`);
        // } else {
        //   alert("Gagal membuat portfolio. Cek koneksi atau hubungi administrator.");
        // }
      }
    },
